...
Source: https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API/Using_the_Web_Animations_API

## [Meet the Web Animations API](#meet_the_web_animations_api)
...
This page contains a suite of examples utilizing the [Web Animations API](/en-US/docs/Web/API/Web_Animations_API), inspired by [Alice in Wonderland](https://en.wikipedia.org/wiki/Alice%27s_Adventures_in_Wonderland "External link (opens in new tab)"). These examples are created by and shared at the courtesy of [Rachel Nabors](https://nearestnabors.com/ "External link (opens in new tab)"). The [full suite of examples](https://codepen.io/collection/nqNJvD "External link (opens in new tab)") are available on CodePen; here we present the ones relevant for our docs.

## [Writing CSS Animations with the Web Animations API](#writing_css_animations_with_the_web_animations_api)
...
### [The CSS version](#the_css_version)
...
This changes Alice's color and her transform's rotation over 3 seconds at a constant (linear) rate and loops infinitely. In the [`@keyframes`](/en-US/docs/Web/CSS/Reference/At-rules/@keyframes) block we can see that 30% of the way through each loop (about .9 seconds in), Alice's color changes from black to a deep burgundy then back again by the end of the loop.

### [Moving it to JavaScript](#moving_it_to_javascript)

Now let's try creating the same animation with the Web Animations API.

#### Representing keyframes

The first thing we need is to create a [Keyframe Object](/en-US/docs/Web/API/Web_Animations_API/Keyframe_Formats) corresponding to our CSS [`@keyframes`](/en-US/docs/Web/CSS/Reference/At-rules/@keyframes) block:
...
Currently, there should be at least two keyframes specified (representing the starting and ending states of the animation sequence). If your keyframe list has only one entry, [`Element.animate()`](/en-US/docs/Web/API/Element/animate) may throw a `NotSupportedError` [`DOMException`](/en-US/docs/Web/API/DOMException) in some browsers until they are updated.
...
- For one, the duration is in milliseconds as opposed to seconds — 3000 not 3s. Like [`setTimeout()`](/en-US/docs/Web/API/Window/setTimeout "setTimeout()") and [`Window.requestAnimationFrame()`](/en-US/docs/Web/API/Window/requestAnimationFrame), the Web Animations API only takes milliseconds.
...
Now it's time to bring them both together with the [`Element.animate()`](/en-US/docs/Web/API/Element/animate) method:
...
## [Controlling playback with play(), pause(), reverse(), and updatePlaybackRate()](#controlling_playback_with_play_pause_reverse_and_updateplaybackrate)
...
### [Pausing and playing animations](#pausing_and_playing_animations)
...
The [`Element.animate()`](/en-US/docs/Web/API/Element/animate) method will immediately run after it is called. To prevent the cake from eating itself up before the user has had the chance to click on it, we call [`Animation.pause()`](/en-US/docs/Web/API/Animation/pause) on it immediately after it is defined, like so:
...
**Note:**
Alternatively, you can define `rabbitDownAnimation` using the [`Animation()`](/en-US/docs/Web/API/Animation/Animation "Animation()") constructor instead, which doesn't start playing until you call `play()`.

We can now use the [`Animation.play()`](/en-US/docs/Web/API/Animation/play) method to run it whenever we're ready. Specifically, we want to link it to a clicking action. We can achieve this via the following:
...
### [Other useful methods](#other_useful_methods)

In addition to pausing and playing, we can use the following Animation methods:

- [`Animation.finish()`](/en-US/docs/Web/API/Animation/finish) skips to the end of the animation.
- [`Animation.cancel()`](/en-US/docs/Web/API/Animation/cancel) aborts the animation and removes its effects.
- [`Animation.reverse()`](/en-US/docs/Web/API/Animation/reverse) sets the animation's playback rate ([`Animation.playbackRate`](/en-US/docs/Web/API/Animation/playbackRate)) to a negative value so it runs backward.

Let's take a look at `playbackRate` first — a negative `playbackRate` will cause an animation to run in reverse.
In [Through the Looking-Glass](https://en.wikipedia.org/wiki/Through_the_Looking-Glass "External link (opens in new tab)"), Alice travels to a world where she must run to stay in place — and run twice as fast to move forward! In the Red Queen's Race example, Alice and the Red Queen are running to stay in place:
...
## [Persisting animation styles](#persisting_animation_styles)
...
A better approach is to use the [`Animation.commitStyles()`](/en-US/docs/Web/API/Animation/commitStyles) method. This writes the computed values of the animation's current styles into its target element's [`style`](/en-US/docs/Web/HTML/Reference/Global_attributes/style) attribute, after which the element can be restyled normally.

## [Automatically removing filling animations](#automatically_removing_filling_animations)
...
- The animation's timeline is monotonically increasing. (This is always true for [`DocumentTimeline`](/en-US/docs/Web/API/DocumentTimeline); other timelines such as [`scroll-timeline`](/en-US/docs/Web/CSS/Reference/Properties/scroll-timeline) can run backwards.)
- The animation is not being controlled by declarative markup such as CSS.
- Every styling effect of the animation's [`AnimationEffect`](/en-US/docs/Web/API/AnimationEffect) is being overridden by another animation that also satisfies all the conditions above. (Typically, when two animations would set the same style property of the same element, the one created last overrides the other.)
...
When the animation is automatically removed, the animation's [`remove`](/en-US/docs/Web/API/Animation/remove_event "remove") event fires.

To prevent the browser from automatically removing animations, call the animation's [`persist()`](/en-US/docs/Web/API/Animation/persist "persist()") method.

The animation's [`replaceState`](/en-US/docs/Web/API/Animation/replaceState "replaceState") property will be `removed` if the animation has been removed, `persisted` if you have called [`persist()`](/en-US/docs/Web/API/Animation/persist "persist()") on the animation, or `active` otherwise.

## [Getting information out of animations](#getting_information_out_of_animations)

Imagine other ways we could use playbackRate, such as improving accessibility for users with vestibular disorders by letting them slow down animations across an entire site. That's impossible to do with CSS without recalculating durations in every CSS rule, but with the Web Animations API, we could use the [`Document.getAnimations`](/en-US/docs/Web/API/Document/getAnimations) method to loop over each animation on the page and halve their `playbackRate`s, like so:
...
## [Callbacks and promises](#callbacks_and_promises)

CSS Animations and Transitions have their own event listeners, and these are also possible with the Web Animations API:

- [`onfinish`](/en-US/docs/Web/API/Animation/finish_event "onfinish") is the event handler for the `finish` event and can be triggered manually with [`finish()`](/en-US/docs/Web/API/Animation/finish "finish()").
- [`oncancel`](/en-US/docs/Web/API/Animation/cancel_event "oncancel") is the event handler for the `cancel` event and can be triggers with [`cancel()`](/en-US/docs/Web/API/Animation/cancel "cancel()").
...
Better still, the Web Animations API also provides a [`finished`](/en-US/docs/Web/API/Animation/finished "finished") promise that will resolve when the animation finishes, or reject if it is canceled.
...
- The [full suite of Alice in Wonderland demos](https://codepen.io/collection/nqNJvD "External link (opens in new tab)") on CodePen for you to play with, fork, and share.
- [Animating like you just don't care with Element.animate](https://hacks.mozilla.org/2016/08/animating-like-you-just-dont-care-with-element-animate/ "External link (opens in new tab)") (2016) Explains the background of the Web Animations API and why it is more performant than other web animation methods.
