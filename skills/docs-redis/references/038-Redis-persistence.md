...
"tableOfContents": {"sections":[{"id":"rdb-advantages","title":"RDB advantages"},{"id":"rdb-disadvantages","title":"RDB disadvantages"},{"id":"aof-advantages","title":"AOF advantages"},{"id":"aof-disadvantages","title":"AOF disadvantages"},{"children":[{"id":"how-it-works","title":"How it works"}],"id":"snapshotting","title":"Snapshotting"},{"children":[{"id":"log-rewriting","title":"Log rewriting"},{"id":"how-durable-is-the-append-only-file","title":"How durable is the append only file?"},{"id":"what-should-i-do-if-my-aof-gets-truncated","title":"What should I do if my AOF gets truncated?"},{"id":"what-should-i-do-if-my-aof-gets-corrupted","title":"What should I do if my AOF gets corrupted?"},{"id":"how-it-works","title":"How it works"},{"id":"how-i-can-switch-to-aof-if-im-currently-using-dumprdb-snapshots","title":"How I can switch to AOF, if I'm currently using dump.rdb snapshots?"}],"id":"append-only-file","title":"Append-only file"},{"id":"interactions-between-aof-and-rdb-persistence","title":"Interactions between AOF and RDB persistence"},{"children":[{"id":"backing-up-aof-persistence","title":"Backing up AOF persistence"}],"id":"backing-up-redis-data","title":"Backing up Redis data"},{"id":"disaster-recovery","title":"Disaster recovery"}]}
,
"codeExamples": []
}
```
...
\* On replicas, RDB supports [partial resynchronizations after restarts and failovers](https://redis.io/docs/latest/operate/oss\_and\_stack/management/replication#partial-resynchronizations-after-restarts-and-failovers).
## RDB disadvantages
...
save 60 1000
This strategy is known as \_snapshotting\_.
### How it works
...
The \_append-only file\_ is an alternative, fully-durable strategy for
...
You can do that by watching `INFO persistence`, waiting for `aof\_rewrite\_in\_progress` and `aof\_rewrite\_scheduled` to be `0`, and validating that `aof\_last\_bgrewrite\_status` is `ok`.
...
and verifying `aof\_rewrite\_in\_progress` is 0. If it's 1, then you'll need to wait for the rewrite to complete.
3. Now you can safely copy the files in the `appenddirname` directory.
4. Re-enable rewrites when done:
[`CONFIG SET`](/commands/config-set) `auto-aof-rewrite-percentage `
...
\* Transfer your snapshots using SCP (part of SSH) to far servers. This is a fairly simple and safe route: get a small VPS in a place that is very far from you, install ssh there, and generate a ssh client key without passphrase, then add it in the `authorized\_keys` file of your small VPS. You are ready to transfer backups in an automated fashion. Get at least two VPS in two different providers
for best results.
