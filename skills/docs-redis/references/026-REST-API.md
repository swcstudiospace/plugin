...
"tableOfContents": {"sections":[{"children":[{"id":"permissions","title":"Permissions"},{"id":"certificates","title":"Certificates"}],"id":"authentication","title":"Authentication"},{"id":"ports","title":"Ports"},{"id":"versions","title":"Versions"},{"children":[{"id":"requests","title":"Requests"},{"id":"responses","title":"Responses"}],"id":"headers","title":"Headers"},{"id":"json-requests-and-responses","title":"JSON requests and responses"},{"id":"response-types-and-error-codes","title":"Response types and error codes"}]}
,
"codeExamples": []
}
...
For more info, see:
- Supported [request endpoints](https://redis.io/docs/latest/operate/rs/references/rest-api/requests), organized by path
- Supported [objects](https://redis.io/docs/latest/operate/rs/references/rest-api/objects), both request and response
- Built-in roles and associated [permissions](https://redis.io/docs/latest/operate/rs/references/rest-api/permissions)
- [Redis Software REST API quick start](https://redis.io/docs/latest/operate/rs/references/rest-api/quick-start) with examples
## Authentication
Authentication to the Redis Software API occurs via [Basic Auth](https://en.wikipedia.org/wiki/Basic\_access\_authentication). Provide your username and password as the basic auth credentials.
...
```
For more examples, see the [Redis Software REST API quick start](https://redis.io/docs/latest/operate/rs/references/rest-api/quick-start).
### Permissions
By default, the admin user is authorized for access to all endpoints. Use [role-based access controls](https://redis.io/docs/latest/operate/rs/security/access-control) and [role permissions](https://redis.io/docs/latest/operate/rs/references/rest-api/permissions) to manage access.
If a user attempts to access an endpoint that is not allowed in their role, the request will fail with a [`403 Forbidden`](https://www.rfc-editor.org/rfc/rfc9110.html#name-403-forbidden) status code. For more details on which user roles can access certain endpoints, see [Permissions](https://redis.io/docs/latest/operate/rs/references/rest-api/permissions).
### Certificates
The Redis Software REST API uses [Self-signed certificates](https://redis.io/docs/latest/operate/rs/security/certificates) to ensure the product is secure. When you use the default self-signed certificates, the HTTPS requests will fail with `SSL certificate problem: self signed certificate` unless you turn off SSL certificate verification.
## Ports
...
If you are using a [Redis Software Docker image](https://redis.io/docs/latest/operate/rs/installing-upgrading/quickstarts/docker-quickstart), run the following command to start the Docker image with port 9443 exposed:
```sh
...
Specify the version in the request [URI](https://en.wikipedia.org/wiki/Uniform\_Resource\_Identifier), as shown in the following table:
| Request path | Description |
|--------------|-------------|
| POST `/v1/bdbs` | A version 1 request for the `/bdbs` endpoint. |
| POST `/v2/bdbs` | A version 2 request for the `/bdbs` endpoint. |
When an endpoint supports multiple versions, each version is documented on the corresponding endpoint. For example, the [bdbs request](https://redis.io/docs/latest/operate/rs/references/rest-api/requests/bdbs/) page documents POST requests for [version 1](https://redis.io/docs/latest/operate/rs/references/rest-api/requests/bdbs/#post-bdbs-v1) and [version 2](https://redis.io/docs/latest/operate/rs/references/rest-api/requests/bdbs/#post-bdbs-v2).
## Headers
### Requests
...
| [403 Forbidden](https://www.rfc-editor.org/rfc/rfc9110.html#name-403-forbidden) | The user cannot access the specified [URI](https://en.wikipedia.org/wiki/Uniform\_Resource\_Identifier). |
| [404 Not Found](https://www.rfc-editor.org/rfc/rfc9110.html#name-404-not-found) | The [URI](https://en.wikipedia.org/wiki/Uniform\_Resource\_Identifier) does not exist. |
