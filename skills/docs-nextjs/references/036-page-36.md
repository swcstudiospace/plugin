...
```sh
node --heap-prof node\_modules/next/dist/bin/next build
```
...
When running the `next build` or `next dev` command, add `NODE\_OPTIONS=--inspect` to the beginning of the command. This will expose the inspector agent on the default port.
