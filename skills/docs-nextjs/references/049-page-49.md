...
"type": "node",
"request": "launch",
"program": "${workspaceFolder}/node\_modules/next/dist/bin/next",
"runtimeArgs": ["--inspect"],
"skipFiles": ["/\*\*"],
"serverReadyAction": {
...
Note that when searching, your source files will have paths starting with `webpack://\_N\_E/./`.
...
To use `--inspect-brk` or `--inspect-wait`, you have to specify `NODE\_OPTIONS` instead. e.g. `NODE\_OPTIONS=--inspect-brk next dev`.
