...
In the [adapter output](/docs/app/api-reference/adapters/output-types), PPR routes are identified by `renderingMode: 'PARTIALLY\_STATIC'` in the prerenders array. Iterate `outputs.prerenders` to find these entries and read `fallback.postponedState`.
...
1. \*\*Read PPR outputs at build time.\*\* In your adapter's `onBuildComplete`, identify prerenders with `renderingMode: 'PARTIALLY\_STATIC'`. Store the shell HTML and `postponedState` in your cache.
