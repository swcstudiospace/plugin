...
[Lazy loading](https://developer.mozilla.org/docs/Web/Performance/Lazy\_loading) in Next.js helps improve the initial loading performance of an application by decreasing the amount of JavaScript needed to render a route.
...
By default, Server Components are automatically [code split](https://developer.mozilla.org/docs/Glossary/Code\_splitting), and you can use [streaming](/docs/app/guides/streaming) to progressively send pieces of UI from the server to the client. Lazy loading applies to Client Components.
## `next/dynamic`
...
{showMore && }
 setShowMore(!showMore)}>Toggle
{/\* Load only on the client side \*/}

)
}
```
> \*\*Note:\*\* When a Server Component dynamically imports a Client Component, automatic [code splitting](https://developer.mozilla.org/docs/Glossary/Code\_splitting) is currently \*\*not\*\* supported.
### Skipping SSR
...
// Runtime will throw MODULE\_NOT\_FOUND if executed
