...
{
source: '/blog',
destination: `${process.env.BLOG\_DOMAIN}/blog`,
},
{
source: '/blog/:path+',
destination: `${process.env.BLOG\_DOMAIN}/blog/:path+`,
},
{
source: '/blog-static/:path+',
destination: `${process.env.BLOG\_DOMAIN}/blog-static/:path+`,
}
];
}
```
...
Links to paths in a different zone should use an `a` tag instead of the Next.js [``](/docs/app/api-reference/components/link) component. This is because Next.js will try to prefetch and soft navigate to any relative path in `` component, which will not work across zones.
## Sharing code
