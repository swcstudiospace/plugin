...
| --- | --- |
| `DB.QueryRow`  `DB.QueryRowContext` | Run a single-row query in isolation. |
| `Tx.QueryRow`  `Tx.QueryRowContext` | Run a single-row query inside a larger transaction. For more, see [Executing transactions](/doc/database/execute-transactions). |
| `Stmt.QueryRow`  `Stmt.QueryRowContext` | Run a single-row query using an already-prepared statement. For more, see [Using prepared statements](/doc/database/prepared-statements). |
| `Conn.QueryRowContext` | For use with reserved connections. For more, see [Managing connections](/doc/database/manage-connections). |
...
| Function | Description |
| --- | --- |
| `DB.Query`  `DB.QueryContext` | Run a query in isolation. |
| `Tx.Query`  `Tx.QueryContext` | Run a query inside a larger transaction. For more, see [Executing transactions](/doc/database/execute-transactions). |
| `Stmt.Query`  `Stmt.QueryContext` | Run a query using an already-prepared statement. For more, see [Using prepared statements](/doc/database/prepared-statements). |
| `Conn.QueryContext` | For use with reserved connections. For more, see [Managing connections](/doc/database/manage-connections). |
