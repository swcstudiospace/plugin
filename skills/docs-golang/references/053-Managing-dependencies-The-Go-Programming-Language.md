...
3. Add your code to a module for dependency tracking (if it isn’t in a module
   already). See [Enabling dependency tracking](#enable_tracking)
4. [Add external packages as dependencies](#adding_dependency) so you can manage
   them.
...
- The *prefix* is typically a string that partially describes the module, such
  ...
  - A name you control.
    ...
    `app`.
- For the *descriptive text*, a good choice would be a project name. Remember
...
2. To add a particular version as a dependency, use the `go get` command as
...
requirements described by the file remain consistent. Use the [`go list`](/ref/mod#go-list-m) command to get the version in use by the current
...
[`stringer`](https://pkg.go.dev/golang.org/x/tools/cmd/stringer) with [`go generate`](/blog/generate), or a specific linter or formatter as part of
