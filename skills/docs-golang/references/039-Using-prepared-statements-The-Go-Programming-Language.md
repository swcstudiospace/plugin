...
| --- | --- |
| `DB.Prepare`  `DB.PrepareContext` | Prepare a statement for execution in isolation or that will be converted to an in-transaction' prepared statement using Tx.Stmt. |
| `Tx.Prepare`  `Tx.PrepareContext`  `Tx.Stmt`  `Tx.StmtContext` | Prepare a statement for use in a specific transaction. For more, see [Executing transactions](/doc/database/execute-transactions). |
| `Conn.PrepareContext` | For use with reserved connections. For more, see [Managing connections](/doc/database/manage-connections). |
