...
[CodeReview](/wiki/CodeReview) wiki page.
Feel free to contribute to the wiki as you learn the review process.

Note that the `gccgo` front end lives elsewhere;
see [Contributing to gccgo](gccgo_contribute.html).
...
- **Step 3**: Register for Gerrit, the code review tool used by the Go team,
  by [visiting this page](https://go-review.googlesource.com/login/).
...
[Before contributing code](#before_contributing).
...
- If you are the copyright holder, you will need to agree to the
  [individual
  contributor license agreement](https://developers.google.com/open-source/cla/individual), which can be completed online.
...
Contributor License Agreements](https://cla.developers.google.com/clas?pli=1&authuser=1) website.
...
to the [`golang-dev`
mailing list](mailto:golang-dev@googlegroups.com).
...
1. Visit [go.googlesource.com](https://go.googlesource.com)
   and click on "Generate Password" in the page's top right menu bar.
   You will be redirected to accounts.google.com to sign in.
2. After signing in, you will be taken to a page with the heading "Configure Git".
...
3. Copy and run this script locally in your terminal to store your secret
...
symbolic link or just copy the executable from `$GOPATH/bin` to this
directory.
...
a new issue](/issue/new) or by claiming
an [existing one](/issues).

### Where to contribute

The Go project consists of the main
[go](https://go.googlesource.com/go) repository, which contains the
...
example, [golang.org/x/pkgsite](https://go.googlesource.com/pkgsite)
is for [pkg.go.dev](https://pkg.go.dev),
[golang.org/x/playground](https://go.googlesource.com/playground)
is for the Go playground, and
[golang.org/x/tools](https://go.googlesource.com/tools) contains
a variety of Go tools, including the Go language server,
[gopls](/s/gopls). You can see a
...
an idea, the [issue tracker](https://github.com/golang/go/issues) is
...
When planning work, please note that the Go project follows a [six-month development cycle](/wiki/Go-Release-Cycle)
...
vendored into
[the standard library](https://github.com/golang/go/blob/master/src/vendor/modules.txt)
and the [`go` command](https://github.com/golang/go/blob/master/src/cmd/vendor/modules.txt).
...
must go
through the
[change proposal process](/s/proposal-process)
...
We encourage using Gerrit to send changes for review. While there is a bit of
a learning curve for users who have only used the GitHub workflow, it is the
primary workflow used by contributors to the Go project and is a smoother and
more full-featured workflow.

The following sections give a brief walkthrough of submitting a change via Gerrit.
For more details on interacting with Gerrit, refer to
[Gerrit's documentation](https://gerrit-review.googlesource.com/Documentation/index.html).
In particular, we recommend reading the
[Review UI Overview](https://gerrit-review.googlesource.com/Documentation/user-review-ui.html)
and [Basic Gerrit Walkthrough — For GitHub Users](https://gerrit-review.googlesource.com/Documentation/intro-gerrit-walkthrough-github.html) pages.

### Overview

This is an overview of the overall process:

- **Step 1:** Clone the source code from `go.googlesource.com` and
  make sure it's stable by compiling and testing it once.

  If you're making a change to the
  [main Go repository](https://go.googlesource.com/go):
...
(defaults to the directory `go` in your home directory).
...
the commit message follows the [suggested format](#commit_messages).
...
You've [written and tested your code](code.html), but
...
The short test suite for the entire tree can be run with `all.bash`
...
you can check the [Build Dashboard](https://build.golang.org).
Test failures may also be caught by the TryBots in code review.

Some repositories, like
[golang.org/x/vscode-go](https://go.googlesource.com/vscode-go) will
...
[Troubleshooting mail errors](#troubleshooting_mail) section.

If your change relates to an open GitHub issue and you have followed the [suggested commit message format](#commit_messages), the issue will be updated in a few minutes by a bot,
...
Although the Gerrit flow is recommended and better supported,
contributors that are already familiar with the
[GitHub flow](https://guides.github.com/introduction/flow/)
are able to use the same process for Go contributions.
Even though Go maintainers use Gerrit for code review,
a tool called [GerritBot](/wiki/GerritBot) has been created to sync
GitHub pull requests to Gerrit.

Open a GitHub pull request as you normally would.
GerritBot will create a corresponding Gerrit change list (a "CL") and post a link to
...
- You will need a [Gerrit account](https://go-review.googlesource.com/login/)
  to respond to your reviewers, including to [mark feedback as 'Done'](#reviews)
  if implemented as suggested.
  It is a good idea to familiarize yourself with Gerrit, such as by
  [skimming the open CLs](https://go-review.googlesource.com/q/status:open+-is:wip),
  subscribing to updates on interesting CLs (via the star icon),
  or [reviewing or giving a +1](/wiki/Gardening#pending-cls) to other people's CLs.
...
- If the request is accepted, all commits will be squashed, and the final
  ...
  See [Writing good commit messages](#commit_messages) for some
  suggestions.
- See the [GerritBot FAQ](/wiki/GerritBot#frequently-asked-questions) for more details.
...
"This change modifies Go to \_\_\_\_\_."
...
See [Commit messages](/wiki/CommitMessage) for additional details.

Add any relevant information, such as benchmark data if the change
affects performance.
The [benchstat](https://godoc.org/golang.org/x/perf/cmd/benchstat)
...
- Change sent during the freeze phase of the development cycle, when the tree
  ...
  In this case,
  a maintainer might review the code with a line such as `R=go1.12`,
  which means that it will be reviewed later when the tree opens for a new
  development window.
  You can add `R=go1.XX` as a comment yourself
...
[Build Dashboard](https://build.golang.org) and check if the same
...
You can also search GitHub issues for the failing error message
or browse [recently updated `watchflakes` issues](https://github.com/golang/go/issues?q=is%3Aissue%20watchflakes%20sort%3Aupdated-desc).
If your change is based on an older commit
or it looks like someone else might have already fixed the problem,
try rebasing to the latest master commit via `git rebase`.
...
(for example, because the [proposal review](/s/proposal-process)
...
In addition to the information here, the Go community maintains a [CodeReview](/wiki/CodeReview) wiki page.
...
the `go` command invoked by `gopls`
must correspond to the version of the source code you are working on.
The `go` command can be built with `make.bash`
and the `bin` directory should be added to your `PATH`.
See [Gopls: Advanced topics](/gopls/advanced) for additional details.
...
that you used during [the registration process](#google_account).
...
Running `all.bash` for every single change to the code tree
is burdensome.
...
- In general, you can run `make.bash` instead of `all.bash`
  to only rebuild the Go tool chain without running the whole test suite.
  Or you
  can run `run.bash` to only run the whole test suite without rebuilding
  the tool chain.
  You can think of `all.bash` as `make.bash`
  followed by `run.bash`.
- In this section, we'll call the directory into which you cloned the Go repository `$GOROOT`.
  The `go` tool built by `$GOROOT/src/make.bash` will be installed
  in `$GOROOT/bin/go` and you
  can invoke it to test your code.
  For instance, if you
  have modified the compiler and you want to test how it affects the
  test suite of your own project, just run `go` `test`
  using it:
...
- If you're modifying the compiler itself, you can just recompile
  the `compile` tool (which is the internal binary invoked
  by `go` `build` to compile each single package).
...
[golang-codereviews@googlegroups.com](https://groups.google.com/group/golang-codereviews)
mailing list.
...
single changes. More details can be found in the [git-codereview documentation](https://pkg.go.dev/golang.org/x/review/git-codereview?tab=doc#hdr-Multiple_Commit_Work_Branches).
