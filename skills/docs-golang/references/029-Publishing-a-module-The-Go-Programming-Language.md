...
1. Open a command prompt and change to your module’s root directory in the local
   repository.
2. Run `go mod tidy`, which removes any dependencies the module might have
...
5. Push the new tag to the origin repository.
   ...
   ```
6. Make the module available by running the [`go list`
...
Developers interested in your module import a package from it and run the `go get` command just as they would with any other module. They can run the `go get` command for latest versions or they can specify a particular version, as
