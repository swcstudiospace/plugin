...
Source: https://platform.claude.com/docs/en/agents-and-tools/mcp-tunnels/security.md
...
\* \*\*Restrict `upstream.allowed\_ips`.\*\* Use the smallest CIDR ranges that cover your MCP servers. This is the [proxy](/docs/en/agents-and-tools/mcp-tunnels/concepts#components)'s primary SSRF defense.
...
Remove the upstream MCP servers from any Managed Agent sessions that use them, and stop passing their URLs in the `mcp\_servers` block of Messages API requests.
