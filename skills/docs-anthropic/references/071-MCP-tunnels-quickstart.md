...
Source: https://platform.claude.com/docs/en/agents-and-tools/mcp-tunnels/quickstart.md
...
cd mcp-tunnel
export TUNNEL\_DOMAIN=YOUR\_TUNNEL\_DOMAIN\_HERE # from step 1
export TUNNEL\_TOKEN='eyJ...' # from step 1
```

```powershell
New-Item -ItemType Directory -Force -Path mcp-tunnel/config, mcp-tunnel/data | Out-Null
Set-Location mcp-tunnel
$env:TUNNEL\_DOMAIN = "YOUR\_TUNNEL\_DOMAIN\_HERE" # from step 1
$env:TUNNEL\_TOKEN = "eyJ..." # from step 1
```
...
if \_\_name\_\_ == "\_\_main\_\_":
mcp.run(transport="streamable-http")
'@ | Set-Content -NoNewline -Encoding ascii -Path hello\_server.py
```

```bash
cat > config/mcp-proxy.yaml < docker-compose.yaml <<'EOF'
services:
mcp-proxy:
...
@"
listen\_addr: ":8080"
tunnel\_domain: $env:TUNNEL\_DOMAIN
tls:
cert\_file: /data/tls.crt
key\_file: /data/tls.key
routes:
