...
Source: https://developers.openai.com/api/docs/guides/chatkit-actions.md
...
def parse\_app\_action(action: Action[str, Any]) -> AppAction:
return ActionAdapter.validate\_python(action)
# Usage in a widget
...
app\_action = parse\_app\_action(action)
if app\_action.type == "example":
await do\_thing(app\_action.payload.id)
yield ThreadItemDoneEvent(
item=AssistantMessageItem(
id=self.store.generate\_item\_id("message", thread, context),
...
onClickAction=ActionConfig(
type="long\_running\_action\_that\_should\_block\_other\_ui\_interactions",
...
| Value | Behavior |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `auto` | The action will adapt to how it’s being used. (\_default\_) |
