...
Source: https://developers.openai.com/api/docs/guides/workload-identity-federation/kubernetes.md
...
def mounted\_service\_account\_token\_provider(token\_path: str) -> SubjectTokenProvider:
def get\_token() -> str:
...
"provider": mounted\_service\_account\_token\_provider(TOKEN\_PATH),
},
)
...
.provider(new MountedServiceAccountTokenProvider(TOKEN\_PATH))
.build();
...
provider = MountedServiceAccountTokenProvider.new(token\_path: TOKEN\_PATH)
