...
Source: https://developers.openai.com/api/docs/guides/developer-mode.md
...
- Disambiguate similar tools: "Prefer `Calendar.create\_event` for meetings; do not use `Reminders.create\_task` for scheduling."
- Specify input shape and sequencing: "First call `Repo.read\_file` with `{ path: "…" }`. Then call `Repo.write\_file` with the modified content. Do not call other tools."
...
alice@example.com and bob@example.com using "Calendar.create\_event".
Do not use any other scheduling tools.
```
```
Create a pull request using "GitHub.open\_pull\_request" from branch
