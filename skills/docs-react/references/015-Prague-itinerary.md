...
Source: https://react.dev/reference/react/useReducer.md
...
\* If the new value you provide is identical to the current `state`, as determined by an [`Object.is`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global\_Objects/Object/is) comparison, React will \*\*skip re-rendering the component and its children.\*\* This is an optimization. React may still need to call your component before ignoring the result, but it shouldn't affect your code.
...
To solve this, you may \*\*pass it as an \_initializer\_ function\*\* to `useReducer` as the third argument instead:
```js {6}
function createInitialState(username) {
// ...
}
...
{state.todos.map(item => (- {item.text}
))}
);
}
```
...
dispatch({ type: 'incremented\_age' }); // Request a re-render with 43
...
```js
const action = { type: 'incremented\_age' };
dispatch(action);
