...
Source: https://react.dev/reference/react/useOptimistic.md
...
handleAddTodo('New todo')}>Add Todo

{optimisticTodos.map(todo => (- {todo.text} {todo.pending && "(Adding...)"}
))}
...
startTransition(async () => {
dispatch({ type: 'update\_quantity', id, quantity });
...
{optimisticCart.map(item => (- {item.name} - ${item.price} ×
  {item.quantity}
...
```
The reducer handles three `action` types (`add`, `remove`, `update\_quantity`) and returns the new optimistic state for each. Each `action` sets a `pending: true` flag so you can show visual feedback while the [Server Function](/reference/rsc/server-functions) runs.
---
