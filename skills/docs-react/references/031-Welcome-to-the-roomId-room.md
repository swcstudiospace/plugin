...
Source: https://react.dev/learn/separating-events-from-effects.md
...
// ...
```
From the user's perspective, \*\*a change to the `message` does \_not\_ mean that they want to send a message.\*\* It only means that the user is typing. In other words, the logic that sends a message should not be reactive. It should not run again only because the reactive value has changed. That's why it belongs in the event handler:
```js {2}
function handleSendClick() {
sendMessage(message);
}
...
# Counter: {count} setCount(0)}>Reset
...
Code inside Effect Events is not reactive. Are there cases in which you would \_want\_ the `setInterval` call to re-run?
