...
Source: https://react.dev/reference/react/useState.md
...
\* If the new value you provide is identical to the current `state`, as determined by an [`Object.is`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global\_Objects/Object/is) comparison, React will \*\*skip re-rendering the component and its children.\*\* This is an optimization. Although in some cases React may still need to call your component before skipping the children, it shouldn't affect your code.
...
![{person.artwork.title}]({person.artwork.image})
);
}
```
```css
...
To solve this, you may \*\*pass it as an \_initializer\_ function\*\* to `useState` instead:
```js
function TodoList() {
const [todos, setTodos] = useState(createInitialTodos);
// ...
...
```
You mutated an existing `obj` object and passed it back to `setObj`, so React ignored the update. To fix this, you need to ensure that you're always [\_replacing\_ objects and arrays in state instead of \_mutating\_ them](#updating-objects-and-arrays-in-state):
```js
