...
Source: https://react.dev/learn/preserving-and-resetting-state.md
...
```
Here, you switch between \_different\_ component types at the same position. Initially, the first child of the `
...
- You could render \_all\_ chats instead of just the current one, but hide all the others with CSS. The chats would not get removed from the tree, so their local state would be preserved. This solution works great for simple UIs. But it can get very slow if the hidden trees are large and contain a lot of DOM nodes.
...
No matter which strategy you pick, a chat \_with Alice\_ is conceptually distinct from a chat \_with Bob\_, so it makes sense to give a `key` to the `<Chat>` tree based on the current recipient.
</DeepDive>
<Recap>
...
<li key={i}>
```
However, you want the state to be associated with \_each particular contact\_.
