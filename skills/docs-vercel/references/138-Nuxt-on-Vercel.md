...
canonical\_url: "https://vercel.com/docs/frameworks/full-stack/nuxt"
last\_updated: 2026-07-03
type: conceptual
prerequisites:
...
Server middleware is best used to read data from or add data to a request's `context`. Doing so allows you to handle authentication or check a request's params, headers, url, [and more](https://www.w3schools.com/nodejs/obj\_http\_incomingmessage.asp).
...
const token = getCookie(event, 'session\_token');
...
event.context.session\_token = token;
}
});
```
...
```
To see your generated image, run your project and use Nuxt DevTools. Or you can visit the image at its URL `/\_\_og-image\_\_/image/og.png`.
