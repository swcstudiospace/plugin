...
canonical\_url: "https://vercel.com/docs/frameworks/full-stack/sveltekit"
...
Vercel provides a set of System Environment Variables that our platform automatically populates. For example, the `VERCEL\_GIT\_PROVIDER` variable exposes the Git provider that triggered your project's deployment on Vercel.
...
The example below exposes `VERCEL\_COMMIT\_REF`, a variable that exposes the name of the branch associated with your project's deployment, to [a `load` function](https://kit.svelte.dev/docs/load) for a Svelte layout:
...
isr: {
expiration: 60,
bypassToken: 'REPLACE\_ME\_WITH\_SECRET\_VALUE',
},
};
```
...
export const config = {
isr: {
// Random token that can be provided to bypass the cached version of the page with a \_\_prerender\_bypass= cookie. Allows rendering content at request time for this route.
...
```
2. Send a `\_\_prerender\_bypass` cookie with the same value as `bypassToken` in your config.
To render the draft content, SvelteKit will check for `\_\_prerender\_bypass`. If its value matches the value of `bypassToken`, it will render content fetched at request time rather than prebuilt content.
