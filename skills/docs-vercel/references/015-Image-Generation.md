...
canonical\_url: "https://vercel.com/docs/ai-gateway/sdks-and-apis/openai-chat-completions/image-generation"
...
if (img.type === 'image\_url' && img.image\_url) {
console.log(`Image ${index + 1}:`, {
size: img.image\_url.url?.length || 0,
preview: `${img.image\_url.url?.substring(0, 50)}...`,
});
}
}
}
...
extra\_body={'modalities': ['text', 'image']},
stream=False,
)
...
if img.get('type') == 'image\_url' and img.get('image\_url'):
image\_url = img['image\_url']['url']
data\_size = len(image\_url) if image\_url else 0
print(f"Image {i+1}: size: {data\_size} chars")
print(f"Preview: {image\_url[:50]}...")
...
- `type`: Always `"image\_url"`
- `image\_url.url`: Base64-encoded data URI of the generated image
### Streaming responses
...
if (img.type === 'image\_url' && img.image\_url) {
console.log(`\n[Image received: ${img.image\_url.url.length} chars]`);
}
}
}
}
...
if img.get('type') == 'image\_url' and img.get('image\_url'):
image\_url = img['image\_url']['url']
print(f"\n[Image received: {len(image\_url)} chars]")
```
