...
canonical\_url: "https://vercel.com/docs/incremental-migration"
...
For example, if you are using [nginx](https://nginx.org/), you can use the [`proxy\_pass`](https://nginx.org/en/docs/http/ngx\_http\_proxy\_module.html#proxy\_pass) directive to send the traffic to the Vercel deployment.
...
server {
listen 80;
server\_name legacy-server.com www.legacy-server.com;
location /feature-path-on-legacy-server {
proxy\_pass https://my-new-app.vercel.app/;
}
}
```
