...
before referring to it in a [Pod](/docs/concepts/workloads/pods/ "A Pod represents a set of running containers in your cluster.").
...
When you first create a [Deployment](/docs/concepts/workloads/controllers/deployment/ "Manages a replicated application on your cluster."),
[StatefulSet](/docs/concepts/workloads/controllers/statefulset/ "A StatefulSet manages deployment and scaling of a set of Pods, with durable storage and persistent identifiers for each Pod."), Pod, or other
...
[kubelet](/docs/reference/command-line-tools-reference/kubelet "An agent that runs on each node in the cluster. It makes sure that containers are running in a pod.") to skip pulling an
...
- Using a *kubelet credential provider* plugin to [dynamically fetch credentials for private registries](#kubelet-credential-provider)
...
[Namespace](/docs/concepts/overview/working-with-objects/namespaces "An abstraction used by Kubernetes to support isolation of groups of resources within a single cluster.") as the
...
This technique can be especially useful for running [static Pods](/docs/tasks/configure-pod-container/static-pod/ "A pod managed directly by the kubelet daemon on a specific node.")
that require container images hosted in a private registry.
Using a [ServiceAccount](/docs/tasks/configure-pod-container/configure-service-account/ "Provides an identity for processes that run in a Pod.") or a
[Secret](/docs/concepts/configuration/secret/ "Stores sensitive information, such as passwords, OAuth tokens, and ssh keys.") to provide private registry credentials
...
[static Pods](/docs/tasks/configure-pod-container/static-pod/ "A pod managed directly by the kubelet daemon on a specific node.") that depend
...
Last modified February 14, 2026 at 4:19 PM PST: [Fix misleading imagePullPolicy: Always description (3136f09689)](https://github.com/kubernetes/website/commit/3136f09689ce6a823330fc740b046784a3af324d)
