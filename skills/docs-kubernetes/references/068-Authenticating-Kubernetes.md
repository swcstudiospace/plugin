...
[ServiceAccounts](/docs/tasks/configure-pod-container/configure-service-account/ "Provides an identity for processes that run in a Pod.").
...
2. The `id_token` can't be revoked, it's like a certificate so it should be short-lived (only a few minutes)
   so it can be very annoying to have to get a new token every few minutes.
3. To authenticate to the Kubernetes dashboard, you must use the `kubectl proxy` command or a reverse proxy
...
- User validation rule expression
  ...
  Refer to the [UserInfo](/docs/reference/generated/kubernetes-api/v1.36/#userinfo-v1-authentication-k8s-io)
...
Last modified September 20, 2025 at 3:38 PM PST: [Revise intro to list of authn methods (0249d0694b)](https://github.com/kubernetes/website/commit/0249d0694b5fd9818c7e5885621cdb3aa4632987)
