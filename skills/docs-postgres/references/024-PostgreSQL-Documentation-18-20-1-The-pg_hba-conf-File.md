...
Supported Versions:
[Current](/docs/current/auth-pg-hba-conf.html "PostgreSQL 18 - 20.1. The pg_hba.conf File")
([18](/docs/18/auth-pg-hba-conf.html "PostgreSQL 18 - 20.1. The pg_hba.conf File"))
/
[17](/docs/17/auth-pg-hba-conf.html "PostgreSQL 17 - 20.1. The pg_hba.conf File")
/
[16](/docs/16/auth-pg-hba-conf.html "PostgreSQL 16 - 20.1. The pg_hba.conf File")
/
[15](/docs/15/auth-pg-hba-conf.html "PostgreSQL 15 - 20.1. The pg_hba.conf File")
/
[14](/docs/14/auth-pg-hba-conf.html "PostgreSQL 14 - 20.1. The pg_hba.conf File")

Development Versions:
[19](/docs/19/auth-pg-hba-conf.html "PostgreSQL 19 - 20.1. The pg_hba.conf File")
/
[devel](/docs/devel/auth-pg-hba-conf.html "PostgreSQL devel - 20.1. The pg_hba.conf File")

Unsupported versions:
[13](/docs/13/auth-pg-hba-conf.html "PostgreSQL 13 - 20.1. The pg_hba.conf File")
/
[12](/docs/12/auth-pg-hba-conf.html "PostgreSQL 12 - 20.1. The pg_hba.conf File")
/
[11](/docs/11/auth-pg-hba-conf.html "PostgreSQL 11 - 20.1. The pg_hba.conf File")
/
[10](/docs/10/auth-pg-hba-conf.html "PostgreSQL 10 - 20.1. The pg_hba.conf File")
/
[9.6](/docs/9.6/auth-pg-hba-conf.html "PostgreSQL 9.6 - 20.1. The pg_hba.conf File")
/
[9.5](/docs/9.5/auth-pg-hba-conf.html "PostgreSQL 9.5 - 20.1. The pg_hba.conf File")
/
[9.4](/docs/9.4/auth-pg-hba-conf.html "PostgreSQL 9.4 - 20.1. The pg_hba.conf File")
/
[9.3](/docs/9.3/auth-pg-hba-conf.html "PostgreSQL 9.3 - 20.1. The pg_hba.conf File")
/
[9.2](/docs/9.2/auth-pg-hba-conf.html "PostgreSQL 9.2 - 20.1. The pg_hba.conf File")
/
[9.1](/docs/9.1/auth-pg-hba-conf.html "PostgreSQL 9.1 - 20.1. The pg_hba.conf File")
/
[9.0](/docs/9.0/auth-pg-hba-conf.html "PostgreSQL 9.0 - 20.1. The pg_hba.conf File")
/
[8.4](/docs/8.4/auth-pg-hba-conf.html "PostgreSQL 8.4 - 20.1. The pg_hba.conf File")
/
[8.3](/docs/8.3/auth-pg-hba-conf.html "PostgreSQL 8.3 - 20.1. The pg_hba.conf File")
/
[8.2](/docs/8.2/auth-pg-hba-conf.html "PostgreSQL 8.2 - 20.1. The pg_hba.conf File")
...
:   This record matches connection attempts made using TCP/IP, but only when the connection is made with SSL encryption.
    ...
    To make use of this option the server must be built with SSL support. Furthermore, SSL must be enabled by setting the [ssl](runtime-config-connection.html#GUC-SSL) configuration parameter (see [Section 18.9](ssl-tcp.html "18.9. Secure TCP/IP Connections with SSL") for more information). Otherwise, the `hostssl` record is ignored except for logging a warning that it cannot match any connections.
...
*`auth-method`*
:   Specifies the authentication method to use when a connection matches this record. The possible choices are summarized here; details are in [Section 20.3](auth-methods.html "20.3. Authentication Methods"). All the options are lower case and treated case sensitively, so even acronyms like `ldap` must be specified as lower case.

    `trust`
    :   Allow the connection unconditionally. This method allows anyone that can connect to the PostgreSQL database server to login as any PostgreSQL user they wish, without the need for a password or any other authentication. See [Section 20.4](auth-trust.html "20.4. Trust Authentication") for details.
    ...
    `scram-sha-256`
    :   Perform SCRAM-SHA-256 authentication to verify the user's password. See [Section 20.5](auth-password.html "20.5. Password Authentication") for details.

    `md5`
    :   Perform SCRAM-SHA-256 or MD5 authentication to verify the user's password. See [Section 20.5](auth-password.html "20.5. Password Authentication") for details.

        ### Warning

        Support for MD5-encrypted passwords is deprecated and will be removed in a future release of PostgreSQL. Refer to [Section 20.5](auth-password.html "20.5. Password Authentication") for details about migrating to another password type.

    `password`
    :   Require the client to supply an unencrypted password for authentication. Since the password is sent in clear text over the network, this should not be used on untrusted networks. See [Section 20.5](auth-password.html "20.5. Password Authentication") for details.

    `gss`
    :   Use GSSAPI to authenticate the user. This is only available for TCP/IP connections. See [Section 20.6](gssapi-auth.html "20.6. GSSAPI Authentication") for details. It can be used in conjunction with GSSAPI encryption.

    `sspi`
    :   Use SSPI to authenticate the user. This is only available on Windows. See [Section 20.7](sspi-auth.html "20.7. SSPI Authentication") for details.

    `ident`
    :   Obtain the operating system user name of the client by contacting the ident server on the client and check if it matches the requested database user name. Ident authentication can only be used on TCP/IP connections. When specified for local connections, peer authentication will be used instead. See [Section 20.8](auth-ident.html "20.8. Ident Authentication") for details.

    `peer`
    :   Obtain the client's operating system user name from the operating system and check if it matches the requested database user name. This is only available for local connections. See [Section 20.9](auth-peer.html "20.9. Peer Authentication") for details.

    `ldap`
    :   Authenticate using an LDAP server. See [Section 20.10](auth-ldap.html "20.10. LDAP Authentication") for details.

    `radius`
    :   Authenticate using a RADIUS server. See [Section 20.11](auth-radius.html "20.11. RADIUS Authentication") for details.

    `cert`
    :   Authenticate using SSL client certificates. See [Section 20.12](auth-cert.html "20.12. Certificate Authentication") for details.

    `pam`
    :   Authenticate using the Pluggable Authentication Modules (PAM) service provided by the operating system. See [Section 20.13](auth-pam.html "20.13. PAM Authentication") for details.

    `bsd`
    :   Authenticate using the BSD Authentication service provided by the operating system. See [Section 20.14](auth-bsd.html "20.14. BSD Authentication") for details.

    `oauth`
    :   Authorize and optionally authenticate using a third-party OAuth 2.0 identity provider. See [Section 20.15](auth-oauth.html "20.15. OAuth Authorization/Authentication") for details.
...
Some examples of `pg_hba.conf` entries are shown in [Example 20.1](auth-pg-hba-conf.html#EXAMPLE-PG-HBA.CONF "Example 20.1. Example pg_hba.conf Entries"). See the next section for details on the different authentication methods.
...
please use
[this form](/account/comments/new/18/auth-pg-hba-conf.html/)
