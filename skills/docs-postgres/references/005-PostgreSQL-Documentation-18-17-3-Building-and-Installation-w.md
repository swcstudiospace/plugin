...
Source: https://www.postgresql.org/docs/current/install-make.html
...
Supported Versions:
[Current](/docs/current/install-make.html "PostgreSQL 18 - 17.3. Building and Installation with Autoconf and Make")
([18](/docs/18/install-make.html "PostgreSQL 18 - 17.3. Building and Installation with Autoconf and Make"))
/
[17](/docs/17/install-make.html "PostgreSQL 17 - 17.3. Building and Installation with Autoconf and Make")
/
[16](/docs/16/install-make.html "PostgreSQL 16 - 17.3. Building and Installation with Autoconf and Make")

Development Versions:
[19](/docs/19/install-make.html "PostgreSQL 19 - 17.3. Building and Installation with Autoconf and Make")
/
[devel](/docs/devel/install-make.html "PostgreSQL devel - 17.3. Building and Installation with Autoconf and Make")
...
3. **Regression Tests**
   ...
   (This won't work as root; do it as an unprivileged user.) See [Chapter 31](regress.html "Chapter 31. Regression Tests") for detailed information about interpreting the test results. You can repeat this test at any later time by issuing the same command.
4. **Installing the Files**

   ### Note

   If you are upgrading an existing system be sure to read [Section 18.6](upgrading.html "18.6. Upgrading a PostgreSQL Cluster"), which has instructions about upgrading a cluster.
...
:   Build with support for LLVM based JIT compilation (see [Chapter 30](jit.html "Chapter 30. Just-in-Time Compilation (JIT)")). This requires the LLVM library to be installed. The minimum required version of LLVM is currently 14.
...
:   Build with support for systemd service notifications. This improves integration if the server is started under systemd but has no impact otherwise; see [Section 18.3](server-start.html "18.3. Starting the Database Server") for more information. libsystemd and the associated header files need to be installed to use this option.
...
:   Build the [uuid-ossp](uuid-ossp.html "F.49. uuid-ossp — a UUID generator") module (which provides functions to generate UUIDs), using the specified UUID library. *`LIBRARY`* must be one of:
...
:   Build with liburing, enabling io\_uring support for asynchronous I/O.
...
:   Build with libxslt, enabling the [xml2](xml2.html "F.50. xml2 — XPath querying and XSLT functionality") module to perform XSL transformations of XML. `--with-libxml` must be specified as well.

`--with-selinux` [#](#CONFIGURE-OPTION-WITH-SEPGSQL)
:   Build with SElinux support, enabling the [sepgsql](sepgsql.html "F.40. sepgsql — SELinux-, label-based mandatory access control (MAC) security module") extension.
...
:   Build without support for the ICU library, disabling the use of ICU collation features (see [Section 23.2](collation.html "23.2. Collation Support")).
...
:   Enable tests using the Perl TAP tools. This requires a Perl installation and the Perl module `IPC::Run`. See [Section 31.4](regress-tap.html "31.4. TAP Tests") for more information.
...
:   Compiles PostgreSQL with support for the dynamic tracing tool DTrace. See [Section 27.5](dynamic-trace.html "27.5. Dynamic Tracing") for more information.
...
please use
[this form](/account/comments/new/18/install-make.html/)
