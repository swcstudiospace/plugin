...
You will normally have much better performance using a [Response Model](../../tutorial/response-model/) than returning a `JSONResponse` directly, as that way it serializes the data using Pydantic, in Rust.
...
When you declare a [Response Model - Return Type](../../tutorial/response-model/) in a path operation, **FastAPI** will use it to serialize the data to JSON, using Pydantic.
