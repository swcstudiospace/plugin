...
Instead of returning a `StreamingResponse` directly, you should probably follow the style in [Stream Data](.././stream-data/), it's much more convenient and handles cancellation behind the scenes for you.

If you are streaming JSON Lines, follow the [Stream JSON Lines](../../tutorial/stream-json-lines/) tutorial.
...
If what you are looking for is performance, you are probably better off using a [Response Model](../../tutorial/response-model/) than an `orjson` response.
