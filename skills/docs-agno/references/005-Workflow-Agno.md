...
| `add_session_state_to_context` | `Optional[bool]` | `None` | If True, add the session\_state to the context of downstream agents and teams |
...
Return the input parameters accepted by the workflow. Callable workflows expose their function signature. Step-based workflows expose a required string `message` parameter. `run_parameters` is a property, so access it without calling it: `workflow.run_parameters`.
**Returns:**
