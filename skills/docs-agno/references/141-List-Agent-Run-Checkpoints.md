...
Source: https://docs.agno.com/api-reference/agents/list-agent-run-checkpoints.md
...
## OpenAPI
````yaml /reference-api/openapi.yaml get /agents/{agent\_id}/runs/{run\_id}/checkpoints
openapi: 3.1.0
info:
...
security: []
paths:
/agents/{agent\_id}/runs/{run\_id}/checkpoints:
get:
tags:
- Agents
...
run. No separate checkpoint table is used; entries are inferred from
message-level checkpoint markers and the terminal end of the transcript.
operationId: list\_agent\_run\_checkpoints
parameters:
- name: agent\_id
in: path
required: true
