...
Source: https://docs.agno.com/api-reference/agents/resume-agent-run-stream.md
...
## OpenAPI
````yaml /reference-api/openapi.yaml post /agents/{agent\_id}/runs/{run\_id}/resume
openapi: 3.1.0
info:
...
security: []
paths:
/agents/{agent\_id}/runs/{run\_id}/resume:
post:
tags:
- Agents
...
operationId: resume\_agent\_run\_stream
parameters:
- name: agent\_id
in: path
required: true
...
schema:
$ref: '#/components/schemas/Body\_resume\_agent\_run\_stream'
responses:
'200':
...
components:
schemas:
Body\_resume\_agent\_run\_stream:
properties:
last\_event\_index:
anyOf:
- type: integer
...
type: object
title: Body\_resume\_agent\_run\_stream
BadRequestResponse:
properties:
detail:
type: string
