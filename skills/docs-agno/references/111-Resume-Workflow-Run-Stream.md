...
Source: https://docs.agno.com/api-reference/workflows/resume-workflow-run-stream.md
...
## OpenAPI
````yaml /reference-api/openapi.yaml post /workflows/{workflow\_id}/runs/{run\_id}/resume
openapi: 3.1.0
info:
...
security: []
paths:
/workflows/{workflow\_id}/runs/{run\_id}/resume:
post:
tags:
- Workflows
...
operationId: resume\_workflow\_run\_stream
parameters:
- name: workflow\_id
in: path
required: true
...
schema:
$ref: '#/components/schemas/Body\_resume\_workflow\_run\_stream'
responses:
'200':
...
Non-admin callers must provide `session\_id`.
content:
application/json:
schema:
...
components:
schemas:
Body\_resume\_workflow\_run\_stream:
properties:
last\_event\_index:
anyOf:
- type: integer
...
type: object
title: Body\_resume\_workflow\_run\_stream
BadRequestResponse:
properties:
detail:
type: string
