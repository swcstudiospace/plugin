...
| `add_search_knowledge_instructions` | `bool` | `True` | If True, add search\_knowledge instructions to the system prompt |
...
- `run_response` (Optional[TeamRunOutput]): The run response to continue
- `run_id` (Optional[str]): The run ID to continue
- `requirements` (Optional[List[Any]]): Updated run requirements to resolve, e.g. tool approvals
...
- `Union[TeamRunOutput, Iterator[Union[TeamRunOutputEvent, RunOutputEvent, TeamRunOutput]]]`: Either a TeamRunOutput or an iterator of events, depending on the `stream` parameter
...
- `Union[TeamRunOutput, AsyncIterator[Union[TeamRunOutputEvent, RunOutputEvent, TeamRunOutput]]]`: Either a TeamRunOutput or an async iterator of events, depending on the `stream` parameter
...
### [​](#get_last_run_output) `get_last_run_output`
...
- `session_state_updates` (Dict[str, Any]): The session state keys and values to update. Provided keys are merged into the existing session state, overwriting existing values
- `session_id` (Optional[str]): Session ID to use (if not provided, the current session is used)
