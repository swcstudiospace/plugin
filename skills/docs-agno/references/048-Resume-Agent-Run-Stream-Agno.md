# Resume Agent Run Stream - Agno

Source: https://docs.agno.com/reference-api/schema/agents/resume-agent-run-stream
...
```
curl --request POST \
  --url https://api.example.com/agents/{agent_id}/runs/{run_id}/resume \
...
fetch('https://api.example.com/agents/{agent_id}/runs/{run_id}/resume', options)
  .then(res => res.json())
...
curl_setopt_array($curl, [
  CURLOPT_URL => "https://api.example.com/agents/{agent_id}/runs/{run_id}/resume",
...
```
HttpResponse<String> response = Unirest.post("https://api.example.com/agents/{agent_id}/runs/{run_id}/resume")
  .header("Authorization", "Bearer <token>")
  .header("Content-Type", "application/x-www-form-urlencoded")
...
url = URI("https://api.example.com/agents/{agent_id}/runs/{run_id}/resume")
