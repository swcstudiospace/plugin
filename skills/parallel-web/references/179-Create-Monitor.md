...
Source: https://docs.parallel.ai/api-reference/monitor/create-monitor.md
...
at
creation, then continues on the configured schedule.
operationId: create\_monitor\_v1\_monitors\_post
requestBody:
required: true
content:
application/json:
...
console.log(monitor.monitor\_id);
components:
schemas:
...
title: Type
description: >-
Type of monitor to create. `event\_stream` monitors a search query
...
`MonitorEventStreamSettings` when `type` is `event\_stream`, and
...
`settings`:
`event\_stream` requires `MonitorEventStreamSettings`, and `snapshot`
...
description: Type-specific settings for an `event\_stream` monitor.
