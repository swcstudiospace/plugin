...
Source: https://docs.parallel.ai/api-reference/tasks/fetch-task-group-runs.md
...
## OpenAPI
````yaml /public-openapi.json get /v1/tasks/groups/{taskgroup\_id}/runs
openapi: 3.1.0
info:
title: Parallel API
...
outputs back in the stream, set the corresponding `include\_input` and
`include\_output` parameters to `true`.
The stream is resumable using the `event\_id` as the cursor. To resume a
stream,
specify the `last\_event\_id` parameter with the `event\_id` of the last
event in the
