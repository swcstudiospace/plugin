...
Source: https://docs.parallel.ai/api-reference/monitor/update-monitor.md
...
## OpenAPI
````yaml /public-openapi.json post /v1/monitors/{monitor\_id}/update
openapi: 3.1.0
info:
title: Parallel API
...
paths:
/v1/monitors/{monitor\_id}/update:
post:
tags:
- Monitor
...
and
`settings` to update type-specific settings on an `event\_stream`
monitor.
At least one field must be provided. Cancelled monitors cannot be
updated.
operationId: update\_monitor\_v1\_monitors\_\_monitor\_id\_\_update\_post
parameters:
- name: monitor\_id
in: path
required: true
...
const monitor = await client.monitor.update('monitor\_id', {
frequency: '12h',
type: 'event\_stream',
settings: { query: 'Extract recent funding news about AI startups' },
});
...
provided; must be `event\_stream` (snapshot monitors have no
...
`event\_stream`. Pass `settings.query` to update the prompt, or
`null` for `settings.advanced\_settings` to clear it.
type: object
...
type-specific settings on an `event\_stream` monitor, include `type` and
`settings`; pass `settings.query` to update the prompt, or `null` for
`settings.advanced\_settings` to clear it. If `settings` is provided,
`type`
...
description: Type-specific update settings for an `event\_stream` monitor.
