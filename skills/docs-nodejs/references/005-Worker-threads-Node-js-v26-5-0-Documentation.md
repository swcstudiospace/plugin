# Worker threads | Node.js v26.5.0 Documentation

Source: https://nodejs.org/api/worker_threads.html

## Worker threads[#](#worker-threads)

**Source Code:** [lib/worker\_threads.js](https://github.com/nodejs/node/blob/main/lib/worker_threads.js)Added in: v10.5.0History

| Version | Changes |
| --- | --- |
| v12.11.0 | This API is no longer experimental. |
| v11.7.0 | This API is no longer behind the `--experimental-worker` CLI flag. |
...
### `worker_threads.getEnvironmentData(key)`[#](#worker_threadsgetenvironmentdatakey)

Added in: v15.12.0, v14.18.0History

| Version | Changes |
| --- | --- |
| v17.5.0, v16.15.0 | No longer experimental. |

- `key` [`<any>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Data_types) Any arbitrary, cloneable JavaScript value that can be used as a
  [`<Map>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Map) key.
...
### `worker_threads.isInternalThread`[#](#worker_threadsisinternalthread)

Added in: v23.7.0, v22.14.0

- Type: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#boolean_type)

Is `true` if this code is running inside of an internal [`Worker`](#class-worker) thread (e.g the loader thread).
...
Is `true` if this code is not running inside of a [`Worker`](#class-worker) thread.
...
- `object` [`<any>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Data_types) Any arbitrary JavaScript value.
...
[`Buffer` pool](buffer.html#static-method-bufferallocunsafesize) with this.
...
### `worker_threads.isMarkedAsUntransferable(object)`[#](#worker_threadsismarkedasuntransferableobject)

Added in: v21.0.0

- `object` [`<any>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Data_types) Any JavaScript value.
...
### `worker_threads.markAsUncloneable(object)`[#](#worker_threadsmarkasuncloneableobject)
...
- `port` [`<MessagePort>`](worker_threads.html#class-messageport) The message port to transfer.
- `contextifiedSandbox` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) A [contextified](vm.html#what-does-it-mean-to-contextify-an-object) object as returned by the
  `vm.createContext()` method.
- Returns: [`<MessagePort>`](worker_threads.html#class-messageport)

Transfer a `MessagePort` to a different [`vm`](vm.html) Context. The original `port`
...
[`<EventTarget>`](events.html#class-eventtarget), and only [`port.onmessage()`](https://developer.mozilla.org/en-US/docs/Web/API/MessagePort/message_event) can be used to receive
events using it.
...
- Type: [`<null>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#null_type) | [`<MessagePort>`](worker_threads.html#class-messageport)

If this thread is a [`Worker`](#class-worker), this is a [`MessagePort`](#class-messageport)
...
### `worker_threads.postMessageToThread(threadId, value[, transferList][, timeout])`[#](#worker_threadspostmessagetothreadthreadid-value-transferlist-timeout)

Added in: v22.5.0, v20.19.0

Stability: 1.1 - Active development

- `threadId` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#number_type) The target thread ID. If the thread ID is invalid, a [`ERR_WORKER_MESSAGING_FAILED`](errors.html#err_worker_messaging_failed) error will be thrown. If the target thread ID is the current thread ID,
  a [`ERR_WORKER_MESSAGING_SAME_THREAD`](errors.html#err_worker_messaging_same_thread) error will be thrown.
- `value` [`<any>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Data_types) The value to send.
- `transferList` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)[] If one or more `MessagePort`-like objects are passed in `value`,
  ...
  See [`port.postMessage()`](#portpostmessagevalue-transferlist) for more information.
- `timeout` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#number_type) Time to wait for the message to be delivered in milliseconds.
  By default it's `undefined`, which means wait forever. If the operation times out,
  a [`ERR_WORKER_MESSAGING_TIMEOUT`](errors.html#err_worker_messaging_timeout) error is thrown.
- Returns: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) A promise which is fulfilled if the message was successfully processed by destination thread.
...
### `worker_threads.receiveMessageOnPort(port)`[#](#worker_threadsreceivemessageonportport)

Added in: v12.3.0History

| Version | Changes |
| --- | --- |
| v15.12.0 | The port argument can also refer to a `BroadcastChannel` now. |

- `port` [`<MessagePort>`](worker_threads.html#class-messageport) | [`<BroadcastChannel>`](worker_threads.html#class-broadcastchannel-extends-eventtarget)
...
### `worker_threads.resourceLimits`[#](#worker_threadsresourcelimits)
...
A special value that can be passed as the `env` option of the [`Worker`](#class-worker)
...
### `worker_threads.setEnvironmentData(key[, value])`[#](#worker_threadssetenvironmentdatakey-value)
...
- `key` [`<any>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Data_types) Any arbitrary, cloneable JavaScript value that can be used as a
  [`<Map>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Map) key.
- `value` [`<any>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Data_types) Any arbitrary, cloneable JavaScript value that will be cloned
...
This value is unique for each [`Worker`](#class-worker) instance inside a single process.
...
On the corresponding worker object (if there is any), it is available as [`worker.threadName`](#workerthreadname).
...
### `worker_threads.locks`[#](#worker_threadslocks)
...
An instance of a [`LockManager`](#class-lockmanager) that can be used to coordinate
...
- `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  - `mode` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#string_type) Either `'exclusive'` or `'shared'`. **Default:** `'exclusive'`.
  - `ifAvailable` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#boolean_type) If `true`, the request will only be granted if the
  ...
  - `steal` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#boolean_type) If `true`, any existing locks with the same name are
    released and the request is granted immediately, pre-empting any queued
    requests. **Default:** `false`.
  - `signal` [`<AbortSignal>`](globals.html#class-abortsignal) that can be used to abort a
    pending (but not yet granted) lock request.
- `callback` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) Invoked once the lock is granted (or immediately with `null` if `ifAvailable` is `true` and the lock is unavailable). The lock is
  released automatically when the function returns, or—if the function returns
  a promise—when that promise settles.
- Returns: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) Resolves once the lock has been released.
...
- `name` [`<any>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Data_types) The name of the channel to connect to. Any JavaScript value
...
- Type: [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) Invoked with a single `MessageEvent` argument
...
- Type: [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) Invoked with a received message cannot be
  deserialized.
...
- `message` [`<any>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Data_types) Any cloneable JavaScript value.
...
- `value` may not contain native (C++-backed) objects other than:
  - [`<CryptoKey>`](webcrypto.html#class-cryptokey)s,
  - [`<FileHandle>`](fs.html#class-filehandle)s,
  - [`<Histogram>`](perf_hooks.html#class-histogram)s,
  - [`<KeyObject>`](crypto.html#class-keyobject)s,
  - [`<MessagePort>`](worker_threads.html#class-messageport)s,
  - [`<net.BlockList>`](net.html#class-netblocklist)s,
  - [`<net.SocketAddress>`](net.html#class-netsocketaddress)es,
  - [`<X509Certificate>`](crypto.html#class-x509certificate)s.
...
`transferList` may be a list of [`<ArrayBuffer>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer), [`MessagePort`](#class-messageport), and
[`FileHandle`](fs.html#class-filehandle) objects.
After transferring, they are not usable on the sending side of the channel
anymore (even if they are not contained in `value`). Unlike with
[child processes](child_process.html), transferring handles such as network sockets is currently
not supported.

If `value` contains [`<SharedArrayBuffer>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/SharedArrayBuffer) instances, those are accessible
...
behind this API, see the [serialization API of the `node:v8` module](v8.html#serialization-api).

##### Considerations when transferring TypedArrays and Buffers[#](#considerations-when-transferring-typedarrays-and-buffers)

All [`<TypedArray>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/TypedArray) | [`<Buffer>`](buffer.html#class-buffer) instances are views over an underlying
[`<ArrayBuffer>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer). That is, it is the `ArrayBuffer` that actually stores
...
See [`Buffer.allocUnsafe()`](buffer.html#static-method-bufferallocunsafesize) for more details on `Buffer` pooling.
...
##### Considerations when cloning objects with prototypes, classes, and accessors[#](#considerations-when-cloning-objects-with-prototypes-classes-and-accessors)
...
not preserved. In particular, [`<Buffer>`](buffer.html#class-buffer) objects will be read as
plain [`<Uint8Array>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Uint8Array)s on the receiving side, and instances of JavaScript
...
- `filename` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#string_type) | [`<URL>`](url.html#the-whatwg-url-api) The path to the Worker's main script or module. Must
  ...
  rather than a path.
- `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  - `argv` [`<any>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Data_types)[] List of arguments which would be stringified and appended to `process.argv` in the worker. This is mostly similar to the `workerData`
  ...
  - `resourceLimits` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) An optional set of resource limits for the new JS
    ...
    - `maxYoungGenerationSizeMb` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#number_type) The maximum size of a heap space for
      recently created objects. If the command-line argument [`--max-semi-space-size`](cli.html#--max-semi-space-sizesize-in-mib) is set, it overrides this setting.
...
#### `worker.cpuUsage([prev])`[#](#workercpuusageprev)

Added in: v24.6.0, v22.19.0

- Returns: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise)

This method returns a `Promise` that will resolve to an object identical to [`process.threadCpuUsage()`](process.html#processthreadcpuusagepreviousvalue),
...
- `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  - `exposeInternals` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#boolean_type) If true, expose internals in the heap snapshot. **Default:** `false`.
...
- Returns: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) A promise for a Readable Stream containing
  a V8 heap snapshot
...
immediately with an [`ERR_WORKER_NOT_RUNNING`](errors.html#err_worker_not_running) error.

#### `worker.getHeapStatistics()`[#](#workergetheapstatistics)

Added in: v24.0.0, v22.16.0

- Returns: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise)

This method returns a `Promise` that will resolve to an object identical to [`v8.getHeapStatistics()`](v8.html#v8getheapstatistics),
or reject with an [`ERR_WORKER_NOT_RUNNING`](errors.html#err_worker_not_running) error if the worker is no longer running.
This methods allows the statistics to be observed from outside the actual thread.
...
The same call as [`perf_hooks` `eventLoopUtilization()`](perf_hooks.html#perf_hookseventlooputilizationutilization1-utilization2), except the values
...
event](#event-exit), then all properties have the value of `0`.
...
See [`port.postMessage()`](#portpostmessagevalue-transferlist) for more details.
...
#### `worker.startCpuProfile([options])`[#](#workerstartcpuprofileoptions)

Added in: v24.8.0

- `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  - `sampleInterval` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#number_type) Requested sampling interval in milliseconds. **Default:** `0`.
  - `maxBufferSize` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#number_type) Maximum number of samples to retain. **Default:** `4294967295`.
- Returns: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise)
...
#### `worker.startHeapProfile([options])`[#](#workerstartheapprofileoptions)

Added in: v24.9.0, v22.20.0

- `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  - `sampleInterval` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#number_type) The average sampling interval in bytes. **Default:** `524288` (512 KiB).
  - `stackDepth` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#number_type) The maximum stack depth for samples. **Default:** `16`.
  - `forceGC` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#boolean_type) Force garbage collection before taking the profile. **Default:** `false`.
...
- Type: [`<stream.Readable>`](stream.html#class-streamreadable)

This is a readable stream which contains data written to [`process.stderr`](process.html#processstderr)
inside the worker thread. If `stderr: true` was not passed to the
[`Worker`](#class-worker) constructor, then data is piped to the parent thread's
[`process.stderr`](process.html#processstderr) stream.

#### `worker.stdin`[#](#workerstdin)

Added in: v10.5.0

- Type: [`<null>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#null_type) | [`<stream.Writable>`](stream.html#class-streamwritable)

If `stdin: true` was passed to the [`Worker`](#class-worker) constructor, this is a
writable stream. The data written to this stream will be made available in
the worker thread as [`process.stdin`](process.html#processstdin).
...
This is a readable stream which contains data written to [`process.stdout`](process.html#processstdout)
inside the worker thread. If `stdout: true` was not passed to the
[`Worker`](#class-worker) constructor, then data is piped to the parent thread's
[`process.stdout`](process.html#processstdout) stream.
...
[`'exit'` event](#event-exit) is emitted.
...
Inside the worker thread, it is available as [`require('node:worker_threads').threadName`](#worker_threadsthreadname).
...
#### `worker[Symbol.asyncDispose]()`[#](#workersymbolasyncdispose)

Added in: v24.2.0, v22.18.0

Calls [`worker.terminate()`](#workerterminate) when the dispose scope is exited.
...
#### Synchronous blocking of stdio[#](#synchronous-blocking-of-stdio)

`Worker`s utilize message passing via [`<MessagePort>`](worker_threads.html#class-messageport) to implement interactions
...
#### Launching worker threads from preload scripts[#](#launching-worker-threads-from-preload-scripts)
