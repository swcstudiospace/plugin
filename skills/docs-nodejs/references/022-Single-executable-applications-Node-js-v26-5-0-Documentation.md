# Single executable applications | Node.js v26.5.0 Documentation
...
### Generating single executable applications with `--build-sea`[#](#-build-sea)
...
[`sea.getRawAsset()`](#seagetrawassetkey) and [`sea.getAssetKeys()`](#seagetassetkeys) APIs for more information.
...
[documentation about startup snapshot support in Node.js](cli.html#--build-snapshot).
...
#### Execution argument extension[#](#execution-argument-extension)
...
Similar to [`sea.getAsset()`](#seagetassetkey-encoding), but returns the result in a [`<Blob>`](buffer.html#class-blob).
An error is thrown when no matching asset can be found.
...
#### Module format of the injected main script[#](#module-format-of-the-injected-main-script)
...
#### Module loading in the injected main script[#](#module-loading-in-the-injected-main-script)
...
#### `require()` in the injected main script[#](#require-in-the-injected-main-script)

`require()` in the injected main script is not the same as the [`require()`](modules.html#requireid)
available to modules that are not injected.
Currently, it does not have any of the properties that non-injected
[`require()`](modules.html#requireid) has except [`require.main`](modules.html#accessing-the-main-module).
...
are equal to [`process.execPath`](process.html#processexecpath).
...
name of [`process.execPath`](process.html#processexecpath).

#### `import.meta` in the injected main script[#](#importmeta-in-the-injected-main-script)
...
#### `import()` in the injected main script[#](#import-in-the-injected-main-script)
...
##### 1. Generating single executable preparation blobs[#](#1-generating-single-executable-preparation-blobs)
...
###### Dumping the preparation blob to disk[#](#dumping-the-preparation-blob-to-disk)
...
##### 2. Injecting the preparation blob into the `node` binary[#](#2-injecting-the-preparation-blob-into-the-node-binary)
...
###### Injecting the preparation blob manually[#](#injecting-the-preparation-blob-manually)
