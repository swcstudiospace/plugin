# Debugger | Node.js v26.5.0 Documentation
...
The debugger supports two modes of operation: [interactive mode](#interactive-mode) and [non-interactive probe mode](#probe-mode).
...
| Version | Changes |
| --- | --- |
| v26.4.0 | Add per-probe `--max-hit <n>` option to limit evaluated hits and finish with a `completed` terminal event as soon as any probe reaches its limit. |
| v26.3.0 | Add `probe_failure` terminal `error` event for inspector-side mid-session failures, and `error.details` for additional context on per-hit and terminal errors. |
| v26.2.0 | JSON report schema bumped to v2. Probe `target` is now `{ suffix, line, column? }` instead of an array. Each "hit" event carries a `location` field reporting the actual evaluation location. When the probe does not specify a column, it binds to the first executable column instead of defaulting to 1. |
...
#### Probe output format[#](#probe-output-format)
...
#### Probing multiple expressions at the same execution point[#](#probing-multiple-expressions-at-the-same-execution-point)
...
#### Selecting the probe location[#](#selecting-the-probe-location)
