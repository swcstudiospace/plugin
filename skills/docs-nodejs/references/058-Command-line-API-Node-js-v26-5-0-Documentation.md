# Command-line API | Node.js v26.5.0 Documentation
...
For more info about `node inspect`, see the [debugger](debugger.html) documentation.
...
When using the [Permission Model](permissions.html#permission-model), the process will not be able to use
...
When using the [Permission Model](permissions.html#permission-model), the process will not be able to spawn any
...
When using the [Permission Model](permissions.html#permission-model), the process will not be able to use FFI
...
When using the [Permission Model](permissions.html#permission-model), the process will not be able to connect
...
When using the [Permission Model](permissions.html#permission-model), the process will not be able to access
network by default.
Attempts to do so will throw an `ERR_ACCESS_DENIED` unless the
...
When using the [Permission Model](permissions.html#permission-model), the process will not be capable of creating
...
When using the [Permission Model](permissions.html#permission-model), the process will not be able to create any
...
Generates a [single executable application](single-executable-applications.html) from a JSON
...
the [single executable application](single-executable-applications.html) documentation.
...
#### `-C condition`, `--conditions=condition`[#](#-conditionscondition)
...
Provide custom [conditional exports](packages.html#conditional-exports) resolution conditions.
...
[`--diagnostic-dir`](#--diagnostic-dirdirectory) command-line option.
...
Set the default value of `order` in [`dns.lookup()`](dns.html#dnslookuphostname-options-callback) and
...
Behavior is the same as [`--env-file`](#--env-filefile), but an error is not thrown if the file
does not exist.
...
variables which configure Node.js](#environment-variables_1), such as `NODE_OPTIONS`,
...
can use the [`--env-file-if-exists`](#--env-file-if-existsfile) flag instead.

#### `-e`, `--eval "script"`[#](#-eval-script)
...
#### `--experimental-config-file=path`, `--experimental-config-file`[#](#--experimental-config-filepath---experimental-config-file)
...
#### `--experimental-ffi`[#](#--experimental-ffi)

Added in: v26.1.0

Stability: 1 - Experimental

Enable the experimental [`node:ffi`](ffi.html) module.
...
#### `--experimental-import-text`[#](#--experimental-import-text)
...
#### `--experimental-package-map=<path>`[#](#--experimental-package-mappath)
...
about [this configuration](single-executable-applications.html#1-generating-single-executable-preparation-blobs) for details.
...
Enable the experimental [`node:stream/iter`](stream_iter.html) module.
...
#### `--experimental-test-tag-filter=<tag>`[#](#--experimental-test-tag-filtertag)
...
filter value to run. See [Test tags](test.html#test-tags) for details on declaring and
inheriting tags.
...
Enable the experimental [`node:vfs`](vfs.html) module.
...
Disable loading native addons that are not [context-aware](addons.html#context-aware-addons).
...
[`--require`](#-r---require-module) and [`--import`](#--importmodule) both run before freezing intrinsics.
...
#### `--inspect-publish-uid=stderr,http`[#](#--inspect-publish-uidstderrhttp)
...
#### `--inspect-wait[=[host:]port]`[#](#--inspect-waithostport)
...
For more information, see [`net.getDefaultAutoSelectFamilyAttemptTimeout()`](net.html#netgetdefaultautoselectfamilyattempttimeout).
...
Disables the use of [`AsyncLocalStorage`](async_context.html#class-asynclocalstorage) backed by `AsyncContextFrame` and
...
Disable using [syntax detection](packages.html#syntax-detection) to determine module type.
...
Disable exposition of [Navigator API](globals.html#navigator) on the global scope.
...
See [Loading ECMAScript modules using `require()`](modules.html#loading-ecmascript-modules-using-require).
...
For more information, see the [TypeScript type-stripping](typescript.html#type-stripping) documentation.
...
#### `--report-dir=directory`, `--report-directory=directory`[#](#--report-dirdirectory---report-directorydirectory)
...
present. See the [test runner execution model](test.html#test-runner-execution-model) section for more information.
...
Regenerates the snapshot files used by the test runner for [snapshot testing](test.html#snapshot-testing).
...
#### `--tls-max-v1.2`[#](#--tls-max-v12)

Added in: v12.0.0, v10.20.0

Set [`tls.DEFAULT_MAX_VERSION`](tls.html#tlsdefault_max_version) to 'TLSv1.2'. Use to disable support for
TLSv1.3.

#### `--tls-max-v1.3`[#](#--tls-max-v13)

Added in: v12.0.0

Set default [`tls.DEFAULT_MAX_VERSION`](tls.html#tlsdefault_max_version) to 'TLSv1.3'. Use to enable support
for TLSv1.3.

#### `--tls-min-v1.0`[#](#--tls-min-v10)

Added in: v12.0.0, v10.20.0

Set default [`tls.DEFAULT_MIN_VERSION`](tls.html#tlsdefault_min_version) to 'TLSv1'. Use for compatibility with
old TLS clients or servers.

#### `--tls-min-v1.1`[#](#--tls-min-v11)

Added in: v12.0.0, v10.20.0

Set default [`tls.DEFAULT_MIN_VERSION`](tls.html#tlsdefault_min_version) to 'TLSv1.1'. Use for compatibility
with old TLS clients or servers.

#### `--tls-min-v1.2`[#](#--tls-min-v12)

Added in: v12.2.0, v10.20.0

Set default [`tls.DEFAULT_MIN_VERSION`](tls.html#tlsdefault_min_version) to 'TLSv1.2'. This is the default for
12.x and later, but the option is supported for compatibility with older Node.js
versions.

#### `--tls-min-v1.3`[#](#--tls-min-v13)

Added in: v12.0.0

Set default [`tls.DEFAULT_MIN_VERSION`](tls.html#tlsdefault_min_version) to 'TLSv1.3'. Use to disable support
...
Prints information about usage of [Loading ECMAScript modules using `require()`](modules.html#loading-ecmascript-modules-using-require).
...
Automatically zero-fills all newly allocated [`Buffer`](buffer.html#class-buffer) instances.
...
Enable the [module compile cache](module.html#module-compile-cache) for the Node.js instance. See the documentation of
[module compile cache](module.html#module-compile-cache) for details.

#### `NODE_COMPILE_CACHE_PORTABLE=1`[#](#node_compile_cache_portable1)

When set to 1, the [module compile cache](module.html#module-compile-cache) can be reused across different directory
...
Disable the [module compile cache](module.html#module-compile-cache) for the Node.js instance. See the documentation of
[module compile cache](module.html#module-compile-cache) for details.
...
##### Source map cache[#](#source-map-cache)
...
### Useful V8 options[#](#useful-v8-options)
...
#### `--abort-on-uncaught-exception`[#](#--abort-on-uncaught-exception-1)

#### `--disallow-code-generation-from-strings`[#](#--disallow-code-generation-from-strings-1)

#### `--enable-etw-stack-walking`[#](#--enable-etw-stack-walking)

#### `--expose-gc`[#](#--expose-gc-1)
...
#### `--max-old-space-size=SIZE` (in MiB)[#](#--max-old-space-sizesize-in-mib)
...
#### `--max-semi-space-size=SIZE` (in MiB)[#](#--max-semi-space-sizesize-in-mib)
