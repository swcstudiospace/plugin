...
When enabling the Permission Model through the [`--permission`](cli.html#--permission)
flag a new property `permission` is added to the `process` object.
This property contains the following functions:

###### `permission.has(scope[, reference])`[#](#permissionhasscope-reference)

API call to check permissions at runtime ([`permission.has()`](process.html#processpermissionhasscope-reference))
...
###### `permission.drop(scope[, reference])`[#](#permissiondropscope-reference)
...
##### Using the Permission Model with `npx`[#](#using-the-permission-model-with-npx)
...
##### Permission Model constraints[#](#permission-model-constraints)
...
##### process.\_debugProcess() and cross-process Inspector activation[#](#process_debugprocess-and-cross-process-inspector-activation)
