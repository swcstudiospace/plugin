# Modules: Packages | Node.js v26.5.0 Documentation
...
along with a reference for the [`package.json`](#nodejs-packagejson-field-definitions) fields defined by Node.js.

### Determining module system[#](#determining-module-system)

#### Introduction[#](#introduction-1)

Node.js will treat the following as [ES modules](esm.html) when passed to `node` as the
...
#### Module resolution and loading[#](#module-resolution-and-loading)
...
using [customization hooks](module.html#customization-hooks).

#### `package.json` and file extensions[#](#packagejson-and-file-extensions)

Within a package, the [`package.json`](#nodejs-packagejson-field-definitions) [`"type"`](#type) field defines how
...
When writing a new package, it is recommended to use the [`"exports"`](#exports) field:
...
##### Path Rules and Validation for Export Targets[#](#path-rules-and-validation-for-export-targets)
...
###### Targets must be relative URLs[#](#targets-must-be-relative-urls)
...
###### No path traversal or invalid segments[#](#no-path-traversal-or-invalid-segments)
...
Added in: v14.6.0, v12.19.0History

| Version | Changes |
| --- | --- |
| v25.4.0, v24.14.0 | Allow subpath imports that start with `#/`. |
...
Added in: v14.13.0, v12.20.0History
...
Added in: v13.2.0, v12.16.0History
...
Added in: v13.1.0, v12.16.0History
...
### Dual CommonJS/ES module packages[#](#dual-commonjses-module-packages)
...
Stability: 1 - Experimental. Enable this API with [`--experimental-package-map`](cli.html#--experimental-package-mappath).
...
- `url` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#string_type) **Required.** An absolute or relative URL. This is parsed using
...
- `dependencies` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) An object mapping bare specifiers to package keys.
...
More details can be found in the [resolution algorithm pseudo-code](modules.html#all-together).
...
#### Multiple packages for the same URL[#](#multiple-packages-for-the-same-url)
...
#### Interaction with other resolution[#](#interaction-with-other-resolution)
...
[self-reference](#self-referencing-a-package-using-its-name) a package using its name.
