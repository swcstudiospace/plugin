# Node-API | Node.js v26.5.0 Documentation
...
recompilation. The [ABI Stability](https://nodejs.org/en/docs/guides/abi-stability/) guide provides a more in-depth explanation.
...
### Writing addons in various programming languages[#](#writing-addons-in-various-programming-languages)
...
[Node-API Media](https://github.com/nodejs/abi-stable-node/blob/HEAD/node-api-media.md) page.
...
#### Enum values in ABI stability[#](#enum-values-in-abi-stability)
...
[node-gyp](https://github.com/nodejs/node-gyp) is a build system based on the [gyp-next](https://github.com/nodejs/gyp-next) fork of
Google's [GYP](https://gyp.gsrc.io) tool and comes bundled with npm. GYP, and therefore node-gyp,
...
[node-pre-gyp](https://github.com/mapbox/node-pre-gyp) is a tool based on node-gyp that adds the ability to
...
[prebuild](https://github.com/prebuild/prebuild) is a tool that supports builds using either node-gyp or
...
[prebuildify](https://github.com/prebuild/prebuildify) is a tool based on node-gyp. The advantage of prebuildify is
...
In order to use the Node-API functions, include the file [`node_api.h`](https://github.com/nodejs/node/blob/HEAD/src/node_api.h) which
...
[Section Agents](https://tc39.es/ecma262/#sec-agents) of the [ECMAScript Language Specification](https://tc39.es/ecma262/) defines the concept
...
See the [Error handling](#error-handling) section for additional information.
...
For more details, review the [Object lifetime management](#object-lifetime-management).
...
An opaque value returned by [`napi_add_async_cleanup_hook`](#napi_add_async_cleanup_hook). It must be passed
...
Function pointer used with [`napi_add_env_cleanup_hook`](#napi_add_env_cleanup_hook). It will be called
...
- `[in] data`: The data that was passed to [`napi_add_env_cleanup_hook`](#napi_add_env_cleanup_hook).

##### `napi_async_cleanup_hook`[#](#napi_async_cleanup_hook)

Added in: v14.10.0, v12.19.0

Function pointer used with [`napi_add_async_cleanup_hook`](#napi_add_async_cleanup_hook). It will be called
when the environment is being torn down.
...
For asynchronous cleanup, [`napi_add_async_cleanup_hook`](#napi_add_async_cleanup_hook) is available.
...
Added in: v14.8.0, v12.19.0**N-API Version:** 8History
...
Added in: v14.8.0, v12.19.0History
...
| Version | Changes |
| --- | --- |
| v25.5.0, v24.13.1 | Added `napi_float16_array` for Float16Array support. |
...
[Section TypedArray objects](https://tc39.es/ecma262/#sec-typedarray-objects) of the [ECMAScript Language Specification](https://tc39.es/ecma262/).
...
[Section Array objects](https://tc39.es/ecma262/#sec-array-objects) of the ECMAScript Language Specification.
...
[Section ArrayBuffer objects](https://tc39.es/ecma262/#sec-arraybuffer-objects) of the ECMAScript Language Specification.
...
[Section Date objects](https://tc39.es/ecma262/#sec-date-objects) of the ECMAScript Language Specification.
...
Added in: v8.0.0**N-API Version:** 1History

| Version | Changes |
| --- | --- |
| v26.2.0 | Added support for `SharedArrayBuffer`. |
...
[Section TypedArray objects](https://tc39.es/ecma262/#sec-typedarray-objects) of the ECMAScript Language Specification.
...
Added in: v8.3.0**N-API Version:** 1History

| Version | Changes |
| --- | --- |
| v25.5.0, v24.13.1 | Added support for `SharedArrayBuffer`. |
...
[Section DataView objects](https://tc39.es/ecma262/#sec-dataview-objects) of the ECMAScript Language Specification.
...
[Section number type](https://tc39.es/ecma262/#sec-ecmascript-language-types-number-type) of the ECMAScript Language Specification.
...
[`Number.MAX_SAFE_INTEGER`](https://tc39.es/ecma262/#sec-number.max_safe_integer) `(2**53 - 1)` will lose precision.
...
[Section string type](https://tc39.es/ecma262/#sec-ecmascript-language-types-string-type) of the ECMAScript Language Specification.
...
#### Functions to convert from Node-API to C types[#](#functions-to-convert-from-node-api-to-c-types)
...
| Version | Changes |
| --- | --- |
| v24.9.0, v22.21.0 | Added support for `SharedArrayBuffer`. |
...
#### Functions to get global instances[#](#functions-to-get-global-instances)
...
### Working with JavaScript values and abstract operations[#](#working-with-javascript-values-and-abstract-operations)
...
[Section ToBoolean](https://tc39.es/ecma262/#sec-toboolean) of the ECMAScript Language Specification.
...
[Section ToObject](https://tc39.es/ecma262/#sec-toobject) of the ECMAScript Language Specification.
...
defined in [Section instanceof operator](https://tc39.es/ecma262/#sec-instanceofoperator) of the ECMAScript Language Specification.
...
as defined in [Section IsArray](https://tc39.es/ecma262/#sec-isarray) of the ECMAScript Language Specification.
...
defined in [Section IsStrctEqual](https://tc39.es/ecma262/#sec-strict-equality-comparison) of the ECMAScript Language Specification.
...
defined in [Section detachArrayBuffer](https://tc39.es/ecma262/#sec-detacharraybuffer) of the ECMAScript Language Specification.
...
[Section SharedArrayBuffer objects](https://tc39.es/ecma262/#sec-sharedarraybuffer-objects) of the ECMAScript Language Specification.
...
the JavaScript function and the data to [`napi_add_finalizer`](#napi_add_finalizer).
...
### Simple asynchronous operations[#](#simple-asynchronous-operations)
...
the [`async_hooks` documentation](async_hooks.html#type) for more information.
...
### Custom asynchronous operations[#](#custom-asynchronous-operations)
...
Added in: v8.6.0**N-API Version:** 1History
...
- The value of `this` is [`global`](globals.html#global) within the script.
...
### Asynchronous thread-safe function calls[#](#asynchronous-thread-safe-function-calls)
...
#### Calling a thread-safe function[#](#calling-a-thread-safe-function)
...
#### Reference counting of thread-safe functions[#](#reference-counting-of-thread-safe-functions)
...
#### Deciding whether to keep the process running[#](#deciding-whether-to-keep-the-process-running)
...
Added in: v10.6.0**N-API Version:** 4History
...
- Version 10 (`NAPI_VERSION` is defined as `10` or higher):
  ...
  [`'uncaughtException'`](process.html#event-uncaughtexception) event, instead of being ignored.
